...
function savePeers(peers) {
  try {
    ...
    console.error("[settings-pro:im] failed to save peer sessions:", error?.message ?? error);
  }
}
...
function emit(sink, method, arg) {
  ...
  try {
    ...
    if (result != null && typeof result.then === "function") {
      result.catch((error) => console.error(`[settings-pro:im] sink.${method} failed:`, error?.message ?? error));
    }
  } catch (error) {
    console.error(`[settings-pro:im] sink.${method} failed:`, error?.message ?? error);
  }
}
...
export class ImBridge {
  ...
  #installQuestionProvider() {
    ...
    if (userQuestions == null) {
      console.warn("[settings-pro:im] userQuestions service unavailable; follow-up questions will not be relayed");
      return;
    }
    const bridge = this;
    const router = {
  ...
  async #sendQuestion(turn, text) {
    ...
    if (typeof onQuestion === "function") {
      try {
        ...
        console.error("[settings-pro:im] failed to send question:", error?.message ?? error);
      }
      return;
    }
  ...
  async #createAgent(peerKey) {
    ...
    const setup = async (agentCtx) => {
      ...
      agentCtx.on("system-prompt/assemble", async (_assembly, _context, next) => {
        ...
        return {
          ...assembled,
          sections: [...(assembled.sections ?? []), { name: "app:settings-pro:im", text: IM_FORMATTING_PROMPT }],
        };
      });
    ...
    if (persistedSessionId !== undefined) {
      try {
        ...
        console.warn(`[settings-pro:im] resume failed for ${peerKey}, creating a fresh session:`, error?.message ?? error);
    ...
    try {
      ...
      console.error("[settings-pro:im] failed to attach session to IM workspace:", error?.message ?? error);
    }
  ...
  async #handleTurn(peerKey, agent, text, sink, lang) {
    ...
    const watchdog = makeWatchdog(this.#replyTimeoutMs, () => {
      timedOut = true;
      console.warn("[settings-pro:im] agent idle timeout; cancelling the active turn");
      try {
        agent.cancel({ kind: "hook", reason: "settings-pro:im idle timeout" });
      } catch (error) {
        console.error("[settings-pro:im] failed to cancel timed-out agent:", error?.message ?? error);
      }
  ...
  async #replyNow(sink, text) {
    ...
    try {
      ...
      console.error("[settings-pro:im] failed to send command reply:", error?.message ?? error);
    }
  }
  ...
  #restartProcess(provider, route, lang) {
    ...
    console.log("[settings-pro:im] restarting dsh web process:", process.execPath, ...args);
    let child;
    try {
      ...
      console.error("[settings-pro:im] failed to launch restart child:", error?.message ?? error);
      return;
    }
    child.on("spawn", () => {
      console.log("[settings-pro:im] restart child launched; exiting");
      saveRestartNotice({ provider, route, lang });
      setTimeout(() => process.exit(0), 250);
    });
    child.on("error", (error) => {
      console.error("[settings-pro:im] restart child failed to launch; staying up:", error?.message ?? error);
    });
    child.unref();
  }
  ...
  async #resetPeer(peerKey) {
    ...
    try {
      ...
      console.error("[settings-pro:im] failed to dispose agent on reset:", error?.message ?? error);
    }
  }
}
