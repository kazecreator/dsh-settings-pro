window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---- Memory panel ----------------------------------------------------
    function useMemory(user) {
      var state = React.useState(null);
      var setData = state[1];
      React.useEffect(function () {
        var disposed = false;
        var load = function () {
          var url = withLang("/settings-pro/memory");
    ...
    function MemoryPanel() {
      ...
      var currentUser = data.user || "owner";
      var summary = data.summary || "";
      var days = data.days || [];

      var withUser = function (path) {
        return withLang(path) + (user ? "&user=" + encodeURIComponent(user) : "");
      };

      var post = function (path, body) {
        return fetch(withUser(path), {
          method: "POST",
      ...
      var exportMd = function () {
        ...
        a.href = withUser("/settings-pro/memory/export.md");
        a.download = "memory-" + currentUser + "-" + new Date().toISOString().slice(0, 10) + ".md";
      ...
      // User selector (only when more than the owner exists).
      if (users.length > 1) {
        var opts = users.map(function (u) {
          return React.createElement("option", { key: u, value: u }, u);
        });
        children.push(React.createElement("div", { key: "user", style: { marginBottom: 12 } },
          React.createElement("span", { style: labelStyle }, zh("User", "用户") + " "),
          React.createElement("select", {
            value: currentUser,
            style: { background: "var(--dsw-alias-bg-layer-2)", color: T.text, border: "1px solid " + T.borderL2, borderRadius: 6, padding: "4px 8px", fontSize: 13 },
    ...
    function PetsPanel() {
      ...
      var headChildren = [
        ...
        React.createElement("div", { key: "d", style: { color: T.textTertiary, fontSize: 12, marginBottom: 12 } },
          zh("When enabled, the pet monitors conversations and jobs and reports progress via status bubbles — it does not create goals or drive the agent.", "开启后，宠物只监控对话与任务，通过状态泡泡报告进度——不会创建目标、也不会自动驱动对话。")),
