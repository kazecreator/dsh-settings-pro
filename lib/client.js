    ...
    function VisionPanel() {
      ...
      var onPick = function (e) {
        ...
        reader.onload = function () {
          ...
          fetch(withLang("/vision/describe"), {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ dataUrl: url, question: "" }),
          })
            .then(function (r) { return r.ok ? r.json() : r.json().then(function (x) { throw new Error(x && x.error ? x.error : "http " + r.status); }); })
            .then(function (d) { setResult(d && d.description ? d.description : ""); })
      ...
      var statusChildren = [
        React.createElement("div", { key: "t", style: titleStyle }, zh("Vision", "视觉")),
        React.createElement("div", { key: "d", style: { color: T.textTertiary, fontSize: 12, marginBottom: 12 } },
          zh("Send an image (Telegram/WeChat/Web) and the assistant will first describe it via DeepSeek's multimodal endpoint, then answer as text.", "发图（Telegram/微信/Web）后，助手会先用 DeepSeek 多模态接口把图片转成文字描述，再按文本作答。")),
        Row({ key: "s1", label: zh("Enabled", "启用"), value: data.enabled ? zh("Yes", "是") : zh("No", "否") }),
        Row({ key: "s2", label: zh("Model", "模型"), value: data.model }),
        Row({ key: "s3", label: zh("Max tokens", "最大输出"), value: String(data.maxTokens) }),
      ];
      children.push(React.createElement("div", { key: "status", style: cardStyle }, statusChildren));

      // Upload a test image.
      children.push(React.createElement("div", { key: "try", style: cardStyle },
        React.createElement("div", { key: "t", style: titleStyle }, zh("Try an image", "试一张图")),
      ...
      if (busy) children.push(React.createElement("div", { key: "busy", style: { color: T.textMuted, fontSize: 13, marginBottom: 8 } }, zh("Analyzing…", "识别中…")));
      ...
      if (result) {
        children.push(React.createElement("div", { key: "res", style: cardStyle },
          React.createElement("div", { key: "t", style: titleStyle }, zh("Description", "识别结果")),
          React.createElement("div", { key: "d", style: { color: T.text, fontSize: 13, whiteSpace: "pre-wrap" } }, result)));
      }
