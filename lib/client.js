window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function UsagePanel() {
      ...
      // platform.deepseek.com", track them leaving to sign in and returning, so
      // we can prompt "click Sync now" the moment they come back to this page.
      ...
      React.useEffect(function () {
        ...
        // Entering "awaiting" means the user just clicked "open" and is about to
        // leave (or already left before this effect registered); treat any
        // subsequent focus/visible event as "came back".
        var gone = true;
      ...
      // the feature is enabled; the button below is a manual re-trigger.
      {
        ...
        if (syncState === "ok") {
          ...
          // through sign-in → return → sync.
          var notSyncedText;
          var notSyncedColor = T.textTertiary;
          if (syncState === "error") {
            notSyncedColor = T.err;
            notSyncedText = sync.reason === "no-session"
              ? zh("No platform login session found. Sign in at platform.deepseek.com, then sync.", "未检测到平台登录会话。请先登录 platform.deepseek.com，再回来同步。")
              : sync.reason === "token-expired"
                ? zh("Your platform login has expired. Sign in again at platform.deepseek.com, then sync.", "平台登录已过期。请重新登录 platform.deepseek.com，再回来同步。")
                : (sync.detail || zh("Sync failed.", "同步失败。"));
          } else {
            notSyncedText = zh("Not synced yet. Sign in at platform.deepseek.com to pull your official billed usage.", "尚未同步。登录 platform.deepseek.com 即可拉取官方计费用量。");
          }
          ...
          // Return-detection: once the user leaves to sign in and comes back,
          // nudge them to click "Sync now" instead of making them re-read docs.
          if (signInPhase === "returned") {
            syncChildren.push(React.createElement("div", { key: "back", style: { color: T.ok, fontSize: 12, fontWeight: 600, marginBottom: 8, lineHeight: "18px" } },
              zh("You're back — click “Sync now” to finish.", "已回到本页 — 点击「立即同步」完成同步。")));
            ...
            syncChildren.push(React.createElement("div", { key: "back", style: { color: T.textTertiary, fontSize: 12, marginBottom: 8, lineHeight: "18px" } },
              zh("Sign in on the opened page, then come back here and click “Sync now”.", "请在打开的页面完成登录，然后回到本页点击「立即同步」。")));
          }
