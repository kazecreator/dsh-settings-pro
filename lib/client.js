window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // the same Enable/Disable button on the right, so the toggle never moves
    // between tabs or between enabled/disabled states.
    function FeatureHeader(props) {
      return React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 } },
        React.createElement("div", { style: { minWidth: 0 } },
          React.createElement("div", { style: Object.assign({}, titleStyle, { marginBottom: 2 }) }, props.label),
          React.createElement("div", { style: { color: T.textTertiary, fontSize: 12 } }, props.enabled ? zh("Enabled", "已启用") : zh("Disabled", "未启用"))),
        React.createElement(P.Button, { variant: props.enabled ? "primary" : "outline", size: "sm", disabled: props.busy, onClick: props.action },
          props.enabled ? zh("Disable", "关闭") : zh("Enable", "开启")));
    }
    ...
    function UsagePanel() {
      ...
      if (data.disabled) {
        return React.createElement("div", { style: { maxWidth: 720 } },
          React.createElement("div", { style: cardStyle },
            React.createElement(FeatureHeader, { label: zh("Usage", "用量"), enabled: false, busy: toggleBusy, action: toggleUsage })));
      }
      ...
      var children = [];
      children.push(React.createElement("div", { key: "head", style: cardStyle },
        React.createElement(FeatureHeader, { label: zh("Usage", "用量"), enabled: true, busy: toggleBusy, action: disableUsage })));
    ...
    function MemoryPanel() {
      ...
      var toggleBusy = toggleBusyState[0];
      var setToggleBusy = toggleBusyState[1];
      var setMemory = function (enabled) {
        setToggleBusy(true);
      ...
      var toggleMemory = function () { setMemory(true); };
      var disableMemory = function () { setMemory(false); };
      ...
      if (data.disabled) {
        return React.createElement("div", { style: { maxWidth: 720 } },
          React.createElement("div", { style: cardStyle },
            React.createElement(FeatureHeader, { label: zh("Memory", "记忆"), enabled: false, busy: toggleBusy, action: toggleMemory })));
      }
      ...
      var headChildren = React.createElement("div", { key: "head", style: cardStyle },
        React.createElement(FeatureHeader, { label: zh("Memory", "记忆"), enabled: true, busy: toggleBusy, action: disableMemory }));
    ...
    // ---- Pets panel ------------------------------------------------------
    // One shared EventSource for the whole client. Every pet hook used to open
    // its own /pets/events stream; with several hooks mounted at once that could
    // exhaust the browser's per-origin HTTP/1.1 connection budget and starve the
    // initial data fetches (leaving the tab stuck on "Loading…"). Fan one stream
    // out to all subscribers instead.
    var petEventsSource = null;
    var petEventsListeners = new Set();
    function subscribePetEvents(fn) {
      petEventsListeners.add(fn);
      if (petEventsSource == null) {
        petEventsSource = new EventSource("/pets/events");
        petEventsSource.onmessage = function (e) {
          var msg;
          try { msg = JSON.parse(e.data); } catch (err) { return; }
          petEventsListeners.forEach(function (cb) {
            try { cb(msg); } catch (err) {}
          });
        };
      }
      return function () { petEventsListeners.delete(fn); };
    }
    ...
    function usePetList() {
      ...
      React.useEffect(function () {
        ...
        load();
        var unsub = subscribePetEvents(function (msg) {
          if (msg.type === "pet-changed" && !disposed) load();
        });
        return function () { disposed = true; unsub(); };
      }, []);
      return [state[0], setData];
    }
    ...
    // On mount it also seeds from `/pets/installs`, so a panel closed mid-install
    // resumes showing the in-flight progress when reopened.
    function useInstallProgress() {
      var state = React.useState({});
      var setProgress = state[1];
      React.useEffect(function () {
        var disposed = false;
        fetch(withLang("/pets/installs"))
          .then(function (r) { return r.ok ? r.json() : null; })
          .then(function (d) {
            ...
            setProgress(function (prev) {
              ...
              var changed = false;
              for (var k in d) {
                if (next[k] != null) continue; // keep fresher SSE data
                next[k] = { phase: d[k] && d[k].phase, percent: d[k] && d[k].percent, error: d[k] && d[k].error };
                changed = true;
              }
    ...
    function usePetApp() {
      ...
      React.useEffect(function () {
        var disposed = false;
        var load = function () {
          fetch(withLang("/pets/app/status"))
            .then(function (r) { return r.ok ? r.json() : null; })
            .then(function (d) { if (d && !disposed) setState(d); })
            .catch(function () {});
        };
        load();
        // Poll so an externally-closed window (running → stopped) reflects fast,
        // and so a reopen mid-install resumes the persisted state.
        var timer = setInterval(load, 4000);
        var unsub = subscribePetEvents(function (msg) {
          if (msg.type === "pet-app-status" && msg.state && !disposed) setState(msg.state);
        });
        return function () { disposed = true; clearInterval(timer); unsub(); };
      }, []);
    ...
    function PetsPanel() {
      ...
      var appPair = usePetApp();
      var appState = appPair[0];
      var setAppState = appPair[1];
      var appBusyState = React.useState(false);
      var appBusy = appBusyState[0];
      var setAppBusy = appBusyState[1];
      ...
      // Head: toggle.
      var headChildren = [
        React.createElement(FeatureHeader, { key: "h", label: zh("Pet", "宠物"), enabled: data.enabled, busy: busy, action: toggle }),
        ...
        React.createElement("div", { key: "note", style: { color: T.textTertiary, fontSize: 12, marginTop: 10, lineHeight: "18px" } },
          zh(
            "Browser mode opens /pet in a browser tab — no extra install. App mode uses the Desktop app below.",
            "网页模式在浏览器标签页打开 /pet，无需额外安装。App 模式使用下方「桌面应用」。")),
      ];
      children.push(React.createElement("div", { key: "head", style: cardStyle }, headChildren));

      // Desktop app: one-click local install + launch, with persistent state.
      var appStatus = appState ? appState.status : "not-installed";
      var appPost = function (path) {
        setAppBusy(true);
        fetch(withLang(path), { method: "POST" })
          .then(function (r) { return r.json().catch(function () { return null; }); })
          .then(function (d) { if (d) setAppState(d); })
          .catch(function () {})
          .finally(function () { setAppBusy(false); });
      };
      var appLabel;
      var appOnClick = null;
      if (appStatus === "installing") {
        appLabel = zh("Installing…", "安装中…");
      } else if (appStatus === "running") {
        appLabel = zh("Stop", "停止");
        appOnClick = function () { appPost("/pets/app/stop"); };
      } else if (appStatus === "installed") {
        appLabel = zh("Launch", "启动");
        appOnClick = function () { appPost("/pets/app/launch"); };
      } else {
        appLabel = zh("Install", "安装");
        appOnClick = function () { appPost("/pets/app/install"); };
      }
      var appStatusLabel = ({
        "not-installed": zh("Not installed", "未安装"),
        "installed": zh("Installed", "已安装"),
        "running": zh("Running", "运行中"),
        "error": zh("Error", "出错"),
      })[appStatus] || null; // "installing" (and unknown) → no side text
      var appChildren = [
        React.createElement("div", { key: "t", style: titleStyle }, zh("Desktop app", "桌面应用")),
        React.createElement("div", { key: "d", style: { color: T.textTertiary, fontSize: 12, lineHeight: "18px" } },
          zh("Run the pet as an always-on-top desktop window (Electron). Installs locally and remembers its state.",
             "把宠物跑成一个置顶的桌面窗口（Electron）。本地安装并记住运行状态。")),
        React.createElement("div", { key: "row", style: { display: "flex", alignItems: "center", gap: 10, marginTop: 10 } },
          React.createElement(P.Button, { variant: appStatus === "installing" ? "outline" : "primary", size: "sm", disabled: appBusy || appStatus === "installing", onClick: appOnClick }, appLabel),
          appStatusLabel ? React.createElement("span", { style: { fontSize: 12, color: appStatus === "error" ? T.err : T.textMuted } }, appStatusLabel) : null),
      ];
      if (appStatus === "error" && appState && appState.error) {
        appChildren.push(React.createElement("div", { key: "e", style: { color: T.err, fontSize: 11, marginTop: 8, wordBreak: "break-word" } }, String(appState.error)));
      }
      children.push(React.createElement("div", { key: "app", style: cardStyle }, appChildren));
        ...
        if (shown.length === 0) {
          ...
          catalogChildren.push(React.createElement("div", { key: "list", style: { maxHeight: 380, overflowY: "auto" } },
            shown.map(function (p) {
              ...
              // A pet is "installing" when this session initiated it OR when the
              // server reports an in-flight install (resumed after reopening the
              // panel). Terminal phases (done/error) fall back to the button.
    ...
    function VisionPanel() {
      ...
      var settingsChildren = [
        React.createElement(FeatureHeader, { key: "enable", label: zh("Vision", "视觉"), enabled: data.enabled, busy: busy, action: toggleVision }),
      ...
      if (busy || err) {
        var msgChildren = [];
        if (busy) msgChildren.push(React.createElement("div", { key: "busy", style: { color: T.textMuted, fontSize: 13, marginBottom: err ? 8 : 0 } }, zh("Working…", "处理中…")));
        if (err) msgChildren.push(React.createElement("div", { key: "err", style: { color: T.err, fontSize: 12, whiteSpace: "pre-wrap" } }, err));
        children.push(React.createElement("div", { key: "msgs", style: cardStyle }, msgChildren));
      }
    ...
    function ImSettingsSection() {
      ...
      return React.createElement("div", null,
        React.createElement("div", { style: cardStyle }, teleChildren),
        React.createElement("div", { style: cardStyle }, wxChildren));
    }
    ...
    function UpdateBadge() {
      ...
      // Inline badge: sits after the label inside a flex row, so the label
      // truncates instead of the NEW chip being squeezed out of the nav cell
      // (long labels like "Settings Pro" leave no room for an absolute chip).
      return React.createElement("span", {
        style: {
          flex: "none",
          padding: "0 5px",
    ...
    function AboutPanel() {
      ...
      function apply() {
        ...
        fetch(withLang("/settings-pro/update/apply"), { method: "POST" })
          ...
          .then(function (d) {
            ...
            // The server relaunches shortly to load the new version. Poll until
            // it's back up and reports no pending update, so the button doesn't
            // sit on "Updating…" until the user switches tabs.
            pollForRestart(0);
          })
          .catch(function (e) {
            setApplying(false);
      ...
      function pollForRestart(attempt) {
        // ~30s budget; the relaunch is quick, but slow machines get headroom.
        if (attempt >= 15) {
          setApplying(false);
          setActionErr(zh("Update installed; refresh the page to finish.", "已安装更新，请刷新页面完成。"));
          return;
        }
        fetch(withLang("/settings-pro/update"))
          .then(function (r) { return r.ok ? r.json() : null; })
          .then(function (d) {
            if (d && d.hasUpdate === false && !d.checkFailed) {
              setSnap(d);
              setApplying(false);
              setActionErr("");
            } else {
              setTimeout(function () { pollForRestart(attempt + 1); }, 2000);
            }
          })
          .catch(function () {
            setTimeout(function () { pollForRestart(attempt + 1); }, 2000);
      ...
      // Single action button: it checks for updates and, once an update is known
      // to exist, becomes the "update & restart" action. One button, one style —
      // no separate high-contrast button whose label could become unreadable.
      var canUpdate = hasUpdate && !isFile;
      var busy = checking || applying;
      var btnLabel = applying
        ? zh("Updating…", "更新中…")
        : checking
          ? zh("Checking…", "检查中…")
          : canUpdate
      ...
      var actionBtn = React.createElement(
        "button",
        {
          key: "action",
          style: {
            padding: "5px 12px", borderRadius: 6, border: "1px solid " + T.borderL2,
            background: "0 0", color: T.text, fontSize: 12, cursor: busy ? "default" : "pointer",
            font: "inherit",
            fontWeight: canUpdate ? 600 : 400,
          },
          disabled: busy,
          onClick: function () { if (canUpdate) apply(); else load(true); },
        },
        btnLabel,
      );
      ...
      var actionCardChildren = [
        React.createElement("div", { key: "actions", style: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" } }, actions),
      ];
      if (statusLine) {
        actionCardChildren.push(React.createElement("div", { key: "status", style: { marginTop: 10 } }, statusLine));
      }
      ...
      return React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 14 } },
        ...
        React.createElement("div", { style: cardStyle }, actionCardChildren),
      );
    }
