  ...
  factory: (require) => {
    ...
    function UsagePanel() {
      ...
      // the feature is enabled; the button below is a manual re-trigger.
      {
        ...
        // Live status line driven by the backend sync outcome.
        if (syncState === "ok") {
          ...
          var reasonText = sync.reason === "no-session"
            ? zh("No platform login session found. Sign in at platform.deepseek.com in a Chromium browser, then sync again.", "未检测到平台登录会话，请在 Chromium 内核浏览器登录 platform.deepseek.com 后重新同步。")
            : sync.reason === "token-expired"
            ? zh("Your platform login has expired. Sign in again at platform.deepseek.com, then sync again.", "平台登录已过期，请重新登录 platform.deepseek.com 后重新同步。")
            : (sync.detail || zh("Sync failed.", "同步失败。"));
          syncChildren.push(React.createElement("div", { key: "st", style: { color: T.err, fontSize: 12, marginBottom: 8, lineHeight: "18px" } }, reasonText));
          ...
          syncChildren.push(React.createElement("div", { key: "st", style: { color: T.textTertiary, fontSize: 12, marginBottom: 8, lineHeight: "18px" } },
            zh("Not synced yet. Sync runs automatically after you enable Usage; you can also sync now.", "尚未同步。开启用量后会自动同步，也可点击下方立即同步。")));
        }
    ...
    // ---- Update banner ---------------------------------------------------
    // Checks the npm registry once when the section opens; offers a one-click
    // update (pnpm add + process restart) when a newer version exists and the
    // plugin is a registry install. Local `file:` checkouts get a muted note
    // instead of a button, since an npm update would replace the dev link.
    function UpdateBanner() {
      ...
      var applying = busy[0];
      var setApplying = busy[1];
      var errState = React.useState("");
      var applyErr = errState[0];
      var setApplyErr = errState[1];
      ...
      if (!snap || snap.checkFailed) return null;
      if (!snap.hasUpdate) return null;

      var isFile = snap.installMode === "file";
      ...
      function apply() {
        ...
        fetch(withLang("/settings-pro/update/apply"), { method: "POST" })
          ...
          .then(function (d) {
            if (!d || d.ok !== true) {
              setApplying(false);
              setApplyErr(zh("Update failed: " + ((d && d.error) || "unknown error"), "更新失败：" + ((d && d.error) || "未知错误")));
            }
            // On success the process restarts; leave the busy state visible.
          })
          .catch(function (e) {
            setApplying(false);
            setApplyErr(zh("Update failed: " + e.message, "更新失败：" + e.message));
          });
      }

      var text = zh(
        "A new version of Settings Pro is available: v" + snap.latest + " (installed v" + snap.current + ")",
        "设置 Pro 有新版本可用：v" + snap.latest + "（当前 v" + snap.current + "）",
      );
      ...
      return React.createElement(
        ...
        React.createElement("div", null,
          ...
          isFile
            ? React.createElement("div", { style: { color: T.textMuted, fontSize: 12, marginTop: 2 } },
                zh("Local development install (file:); run pnpm add manually to switch to the npm version.", "本地开发安装（file:），如要切到 npm 版本请手动执行 pnpm add。"))
            : null,
          applyErr ? React.createElement("div", { style: { color: T.err, fontSize: 12, marginTop: 2 } }, applyErr) : null,
        ),
        isFile || applying
          ? null
          : React.createElement("button", { style: btnStyle, onClick: apply }, zh("Update & Restart", "更新并重启")),
        applying
          ? React.createElement("div", { style: { color: T.textMuted, fontSize: 12, whiteSpace: "nowrap" } },
              zh("Updating… restarting", "更新中…即将重启"))
          : null,
      );
    }
    ...
    function SettingsProSection() {
      ...
      var children = [
        React.createElement(UpdateBanner, { key: "update" }),
        React.createElement(
          "div",
