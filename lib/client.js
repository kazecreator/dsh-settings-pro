window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // The DSH system language (locale.preference) is authoritative for the tab
    // names and section title. It is fetched once from the server and applied
    // over the browser guess; listeners re-render the settings section live.
    var uiLangListeners = [];
    ...
    fetch(withLang("/settings-pro/status"))
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (d) { if (d && d.lang) setUiLang(d.lang); })
      .catch(function () {});
    function zh(en, zh) {
    ...
    function UsagePanel() {
      ...
      var backfillMsgState = React.useState("");
      var backfillMsg = backfillMsgState[0];
      var setBackfillMsg = backfillMsgState[1];
      ...
      // Cost and tokens come purely from the official backfill; the live local
      // token store is only a fallback before the first backfill completes.
      var chartDays;
      if ((data.officialDaily || []).length > 0) {
        chartDays = (data.officialDaily || []).map(function (o) {
          return {
            date: o.date,
            cost: o.cost || 0,
        ...
        chartDays = (data.daily || []).map(function (d) {
          return {
            date: d.date,
            cost: (d.total && d.total.cost) || 0,
            inputTokens: (d.total && d.total.inputTokens) || 0,
            cacheReadTokens: (d.total && d.total.cacheReadTokens) || 0,
            outputTokens: (d.total && d.total.outputTokens) || 0,
            models: d.models,
          };
        });
      }
      ...
      var todayDate = data.today && data.today.date;
      ...
      if (chartDays.length === 0) {
        chartChildren.push(React.createElement("div", { key: "empty", style: { color: T.textMuted, fontSize: 12 } }, zh("No usage yet", "暂无用量")));
      } else {
        var maxCost = 0;
      ...
      // Backfill of accurate history + token breakdown; the token is remembered
      // so a later re-sync works without pasting again. Always visible so the
      // re-sync action stays reachable.
      {
        var syncAuto = function () {
          ...
          return fetch(withLang("/settings-pro/usage/backfill"), {
            ...
            .catch(function (x) { setBackfillMsg(x && x.message ? x.message : String(x)); })
            .finally(function () { setBackfillBusy(false); });
        };

        var hasToken = data.hasPlatformToken === true;
        var syncChildren = [React.createElement("div", { key: "t", style: titleStyle }, zh("Sync official usage", "同步官方用量"))];
        // Auto-sync (primary): reads the browser session, falling back to the
        // saved token — covers both first sync and re-sync.
        syncChildren.push(React.createElement("div", { key: "auto", style: { display: "flex", gap: 8, alignItems: "center", marginBottom: 8 } },
          React.createElement(P.Button, { variant: "primary", size: "sm", disabled: backfillBusy, onClick: syncAuto }, zh("Auto sync", "自动同步")),
          React.createElement("span", { style: { color: T.textTertiary, fontSize: 12 } },
            zh("Pulls platform-billed cost and token breakdown — no manual entry.", "从 DeepSeek 平台拉取官方计费的成本与 token 明细，无需手动操作。"))));
        syncChildren.push(React.createElement("div", { key: "compat", style: { color: T.textTertiary, fontSize: 12, marginTop: 6, lineHeight: "18px" } },
          zh(
            "Auto-sync reads the sign-in session from a Chromium-based browser (Chrome / Edge / Brave / Arc / Opera on macOS / Windows / Linux). Firefox / Safari are not supported — paste the userToken manually instead.",
            "自动同步读取本机 Chromium 内核浏览器（Chrome / Edge / Brave / Arc / Opera，macOS / Windows / Linux）的登录会话；Firefox / Safari 不支持，请手动粘贴 userToken。")));
        if (hasToken) {
          syncChildren.push(React.createElement("div", { key: "d", style: { color: T.textTertiary, fontSize: 12, marginTop: 4 } },
            zh("A saved login credential is reused automatically.", "已保存登录凭据，会自动复用。")));
        }
        if (backfillMsg) {
          syncChildren.push(React.createElement("div", { key: "msg", style: { color: T.err, fontSize: 12, marginTop: 8, whiteSpace: "pre-wrap" } }, backfillMsg));
          // Only surface the platform link after a failed sync (token expired
          // or no browser session), when a re-login is actually needed.
          syncChildren.push(React.createElement("div", { key: "open", style: { marginTop: 8 } },
            React.createElement("a", {
              href: "https://platform.deepseek.com/usage",
              target: "_blank",
              rel: "noopener noreferrer",
              style: { color: T.textTertiary, fontSize: 12, textDecoration: "underline" },
    ...
    function petStatusText(data) {
      ...
      } else if (data.currentTodo) {
        parts.push(React.createElement("div", { key: "goal", style: { whiteSpace: "pre-line", overflowWrap: "break-word" } }, "📋 " + String(data.currentTodo).trim()));
      }
    ...
    function PetsPanel() {
      ...
      var catalogChildren = [React.createElement("div", { key: "t", style: titleStyle }, zh("Online library", "在线库") + " · " + catalogList.length)];
      if (!catalog) {
        if (catalogErr) {
            ...
            shown.map(function (p) {
              ...
              return React.createElement("div", { key: p.id, style: { display: "flex", alignItems: "center", gap: 10, padding: "6px 0", borderBottom: "1px solid " + T.borderL2 } },
                React.createElement("div", { style: { flex: 1, minWidth: 0 } },
                  ...
                  React.createElement("div", { style: { color: T.textTertiary, fontSize: 11 } }, p.category + (p.author ? " · " + p.author : ""))),
                button);
            })));
        }
      }
