 ...
 */
async function fetchWithRetry(url, { timeoutMs = 20000, headers } = {}) {
  let attempt = 0;
  for (;;) {
...
export async function installPet(dir, slug, onProgress) {
  ...
  const petJsonRes = await fetchWithRetry(`${base}/pet.json`, { timeoutMs: 20000 });
  ...
  const sheetRes = await fetchWithRetry(`${base}/spritesheet.webp`, { timeoutMs: 60000 });
