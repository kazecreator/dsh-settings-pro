...
/** Max attempts (initial + retries) for a pet/catalog fetch that hits HTTP 429. */
const PET_FETCH_MAX_ATTEMPTS = 3;
/** Base backoff for a 429 without a `Retry-After` header (ms). */
const PET_FETCH_BASE_DELAY_MS = 1000;
/** Ceiling for any 429 retry delay, so a huge `Retry-After` cannot hang install (ms). */
const PET_FETCH_MAX_DELAY_MS = 10000;
const COLS = 8;
const CELL_W = 192;
const CELL_H = 208;
...
function catalogFetchedAtPath(dir) {
  return join(dir, "pets", "codex-catalog-fetched-at.json");
}

/** Timestamp (epoch ms) of the last successful catalog fetch, or 0. */
export function readCatalogFetchedAt(dir) {
  try {
    const parsed = JSON.parse(readFileSync(catalogFetchedAtPath(dir), "utf8"));
    const ts = Number(parsed?.fetchedAt);
...
function writeCatalogFetchedAt(dir, ts) {
  try {
    mkdirSync(join(dir, "pets"), { recursive: true });
    writeFileSync(catalogFetchedAtPath(dir), JSON.stringify({ fetchedAt: ts }) + "\n");
  } catch {
...
/**
 * Fetch with a bounded retry on HTTP 429. `raw.githubusercontent.com` rate
 * limits per-IP request bursts (returning 429 with a `Retry-After` header in
 * seconds); honor that header when present, otherwise back off exponentially.
 * Each attempt gets a fresh timeout so a slow attempt never eats the retry
 * budget. Returns the final `Response` (the caller reads/streams the body).
 */
async function fetchWithRetry(url, { timeoutMs = 60000, headers } = {}) {
  let attempt = 0;
  for (;;) {
    ...
    if (res.status === 429 && attempt < PET_FETCH_MAX_ATTEMPTS - 1) {
      const retryAfter = Number(res.headers.get("retry-after"));
      const delayMs = Number.isFinite(retryAfter) && retryAfter > 0
        ? Math.min(retryAfter * 1000, PET_FETCH_MAX_DELAY_MS)
        : PET_FETCH_BASE_DELAY_MS * 2 ** attempt;
      attempt += 1;
      console.warn(`[settings-pro:pets] ${url} rate-limited (429); retrying in ${delayMs}ms (attempt ${attempt}/${PET_FETCH_MAX_ATTEMPTS - 1})`);
      await sleep(delayMs);
      continue;
    }
    return res;
...
export async function fetchCatalog(dir) {
  ...
  if (!Array.isArray(list)) throw new Error("catalog has an unexpected format");
  mkdirSync(join(catalogPath(dir), ".."), { recursive: true });
  writeFileSync(catalogPath(dir), JSON.stringify(list));
  writeCatalogFetchedAt(dir, Date.now());
  return list;
}
...
export async function installPet(dir, slug, onProgress) {
  ...
  if (clean === "") throw new Error("missing pet id");
  const base = `${RAW_BASE}/pets/${encodeURIComponent(clean)}`;

  onProgress?.({ phase: "fetching", loaded: 0, total: 0 });

  const petJsonRes = await fetchWithRetry(`${base}/pet.json`, { timeoutMs: 60000 });
  if (petJsonRes.status === 404) throw new Error("Pet not found");
  if (!petJsonRes.ok) throw new Error(`pet.json HTTP ${petJsonRes.status}`);
  const petJson = await petJsonRes.json();

  const sheetRes = await fetchWithRetry(`${base}/spritesheet.webp`, { timeoutMs: 300000 });
  if (sheetRes.status === 404) throw new Error("Pet not found");
  ...
  if (sheetBytes.length === 0) throw new Error("spritesheet is empty");
