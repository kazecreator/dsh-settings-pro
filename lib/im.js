...
import { imStoragePath, migrateLegacyImStorage } from "./im-storage.js";

const LOG = "[settings-pro:im]";

/**
 * Path of the UI-written runtime overrides (survive restarts, override the
 * patch layer). Lives under the plugin's own `storages/dsh-settings-pro/im`
 * root; state left behind by the archived dsh-im package is migrated once on
 * startup (see `migrateLegacyImStorage`).
 */
