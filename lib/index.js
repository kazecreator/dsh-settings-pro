...
function formatUsageText(payload) {
  ...
  if (officialToday && (officialToday.cost > 0 || officialToday.cacheHit > 0 || officialToday.cacheMiss > 0 || officialToday.response > 0)) {
    ...
    if (total) {
      lines.push(
        `Today's usage: input ${total.inputTokens} (cache-hit ${total.cacheReadTokens}) / output ${total.outputTokens} tokens, cost ≈ ¥${Number(total.cost).toFixed(2)}`,
      );
    }
  }
  return lines.join("\n") || "No usage data yet";
        ...
        readJson(req)
          .then(async (body) => {
            ...
            if (token === "") {
              throw new Error(
                body?.auto === true
                  ? "No platform login session found in the browser; sign in at platform.deepseek.com first, then click Auto-sync"
