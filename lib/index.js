    ...
    try {
      ...
      if (resolved.memoryEnabled !== false) {
        ...
        registerRoute(ctx, "/settings-pro/memory/export.md", (req, res) => {
          ...
          const markdown = memory.exportMarkdown();
          res.writeHead(200, {
            "content-type": "text/markdown; charset=utf-8",
            "cache-control": "no-cache",
            "content-disposition": `attachment; filename="memory-${new Date().toISOString().slice(0, 10)}.md"`,
          });
          res.end(markdown);
        });
        ...
        registerRoute(ctx, "/settings-pro/memory/summary", (req, res) => {
          ...
          readJson(req)
            .then((body) => {
              memory.setSummary(body?.summary ?? "");
              sendJson(res, memory.exportJson());
            })
        ...
        registerRoute(ctx, "/settings-pro/memory/note", (req, res) => {
          ...
          readJson(req)
            .then((body) => {
              memory.addNote(body?.text ?? "", { sessionId: "manual" });
              sendJson(res, memory.exportJson());
            })
            .catch((error) => sendJson(res, { error: error?.message ?? String(error) }, 400));
        });
      ...
      const vision = new VisionService(ctx, resolved);

      // --- vision (image → text description via DeepSeek multimodal) ---
      registerRoute(ctx, "/vision/status", (req, res) => {
        ...
        sendJson(res, {
          ...
          model: resolved.visionModel ?? "deepseek-v4-pro",
      ...
      registerRoute(ctx, "/pets/active", (req, res) => {
        ...
        sendJson(res, petStore.get(petStore.active()) ?? petStore.list()[0]);
      });
