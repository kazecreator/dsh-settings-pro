...
import { applyUpdate, restartProcess, updateSnapshot } from "./update.js";
 ...
 * settles, so the plugin degrades gracefully in profiles that lack a
 * particular seam.
 */
const inject = [];
...
async function describeImageBlock(attachments, vision, block) {
  try {
    ...
    return text !== "" ? `[Image content] ${text}` : "[Image]";
  } catch {
    return "[图片]";
  }
}
...
function formatUsageText(payload) {
  ...
  if (Array.isArray(infos) && infos.length > 0) {
    for (const b of infos) {
      lines.push(
        `Balance (${b.currency}): total ${b.total_balance}, granted ${b.granted_balance}, topped up ${b.topped_up_balance}`,
      );
    }
  } else if (payload?.balanceError) {
    lines.push(`Balance query failed: ${payload.balanceError}`);
  }
  // Official billed usage is the only source — no local estimate fallback.
  ...
  if (officialToday && (officialToday.cost > 0 || officialToday.cacheHit > 0 || officialToday.cacheMiss > 0 || officialToday.response > 0)) {
    ...
    lines.push(
      `Today's usage: input cache-hit ${hit} / miss ${miss} / output ${resp} tokens, cost ¥${Number(officialToday.cost || 0).toFixed(2)}`,
    );
  } else {
    lines.push("Official usage not synced yet — sign in to platform.deepseek.com, then sync");
  }
  return lines.join("\n");
}
...
function registerUsageTool(ctx, usage) {
  ...
  tools.register(
    defineTool({
      name: "get_usage",
      description: "Query DeepSeek account balance and locally aggregated daily token usage/cost (peak/off-peak pricing).",
      parameters: {},
      output: {
        schema: { type: "string" },
      ...
      async execute(_args, _exec) {
        if (!usage.enabled) return "Usage is disabled. Enable it in Settings Pro → Usage.";
...
function registerMemoryTools(ctx, memory, enabled) {
  ...
  tools.register(
    defineTool({
      name: "read_memory",
      description: "Read cross-restart persistent memory (last-session summary + project progress notes).",
      parameters: {},
      output: {
        schema: { type: "string" },
        render: (_args, value) => [{ type: "text", text: value }],
      ...
      async execute(_args, _exec) {
        if (!enabled.value) return "Memory is disabled. Enable it in Settings Pro → Memory.";
        return memory.summaryText() || "(no memory yet)";
      },
    }),
  );
  ...
  tools.register(
    defineTool({
      name: "write_memory",
      description: "Write a cross-restart persistent memory entry (project progress / conclusions / todos); new sessions auto-inject it.",
      parameters: {
        text: { type: "string", required: true, description: "Content to remember" },
        summary: { type: "string", description: "Optional: replace the whole memory summary" },
      },
      output: {
        schema: { type: "string" },
        render: (_args, value) => [{ type: "text", text: value }],
...
function apply(ctx, config) {
  ...
  const start = () => {
    try {
      ...
      // Reuse the previously saved platform token for auto-sync, so a transient
      // browser-session read failure never wrongly reports "no session" and
      // sends the user off to re-sign-in while their token is still valid.
      const savedPlatformToken = String(loadRuntimeConfig().deepseekPlatformToken ?? "").trim();
      if (savedPlatformToken !== "") usage.platformToken = savedPlatformToken;
      usage.start();
      ...
      registerRoute(ctx, "/settings-pro/usage/backfill", (req, res) => {
        ...
        readJson(req)
          .then(async (body) => {
            ...
            if (token === "") {
              usage.markSyncError("no-session");
              throw new Error(
                body?.auto === true
                  ? "No platform login session found in the browser; sign in at platform.deepseek.com first, then sync again"
                  : "Please provide the platform userToken",
              );
            }
            try {
              ...
              usage.markSyncError(
                /invalid|expired|401|40002|40003/i.test(message) ? "token-expired" : "failed",
                message,
              );
              throw error;
            }
      ...
      const petApp = new PetAppManager(dir);
      ...
      registerRoute(ctx, "/vision/describe", (req, res) => {
        ...
        readJson(req, 12 * 1024 * 1024)
          .then((body) => {
            ...
            if (!m) throw new Error("Missing dataUrl image data");
            const bytes = Buffer.from(m[2], "base64");
            return vision.describe(bytes, { mediaType: m[1].toLowerCase(), question: body?.question ?? "", lang: body?.lang === "zh" ? "zh" : "en" });
          })
      ...
      // In-flight Codex installs, keyed by pet id. Kept server-side so a
      // settings panel that was closed mid-install can resume showing progress
      // when it reopens (the SSE stream alone is ephemeral and connection-bound).
      const petInstalls = new Map();
      ...
      petApp.onStateChange = (state) => broadcastPetEvent({ type: "pet-app-status", state });
      ...
      // Online Codex pet catalog (Awesome Codex Pet). Fetched on first view,
      // auto-refreshed when the cache is older than a week, and refreshable on
      // demand via the "Refresh" button.
      const CATALOG_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000;
      const catalogPayload = (list) => {
        const installed = new Set(petStore.listCodex().map((p) => p.id));
        return {
          fetchedAt: readCatalogFetchedAt(dir),
          pets: list.map((entry) => ({
            id: entry.slug,
      ...
      registerRoute(ctx, "/pets/catalog", async (req, res) => {
        ...
        try {
          ...
          const fetchedAt = readCatalogFetchedAt(dir);
          const fresh = list != null && fetchedAt > 0 && (Date.now() - fetchedAt) < CATALOG_MAX_AGE_MS;
          if (!fresh) {
            // First view, or the weekly auto-refresh window elapsed: re-pull.
            // On failure, fall back to the cached copy when one exists.
            try {
              list = await fetchCatalog(dir);
            } catch (error) {
              if (list == null) throw error;
            }
          }
          sendJson(res, catalogPayload(list));
        } catch (error) {
          sendJson(res, { error: error?.message ?? String(error) }, 500);
        }
      });

      registerRoute(ctx, "/pets/catalog/refresh", async (req, res) => {
        if (req.method !== "POST") {
          res.writeHead(405);
          res.end();
          return;
        ...
        try {
          const list = await fetchCatalog(dir);
          sendJson(res, catalogPayload(list));
        } catch (error) {
          sendJson(res, { error: error?.message ?? String(error) }, 500);
        }
        ...
        readJson(req)
          .then(async (body) => {
            ...
            const report = (phase, percent, error) => {
              petInstalls.set(id, { phase, percent, error });
      ...
      // Current in-flight installs, so a reopened settings panel can resume
      // showing live progress instead of losing it after it was closed.
      registerRoute(ctx, "/pets/installs", (req, res) => {
        if (req.method !== "GET" && req.method !== "HEAD") {
          res.writeHead(405);
          res.end();
          return;
        ...
        const out = {};
        for (const [petId, state] of petInstalls) out[petId] = state;
        sendJson(res, out);
      });

      // --- local desktop pet app (install / launch / stop, persistent state) ---
      registerRoute(ctx, "/pets/app/status", (req, res) => {
        if (req.method !== "GET" && req.method !== "HEAD") {
          res.writeHead(405);
          res.end();
          return;
        }
        sendJson(res, petApp.status());
      });
      registerRoute(ctx, "/pets/app/install", (req, res) => {
        if (req.method !== "POST") {
          res.writeHead(405);
          res.end();
          return;
        }
        // Kick off the (slow) install and answer immediately; progress arrives
        // via the pet-app-status SSE stream. A re-open mid-install resumes from
        // the persisted state.
        petApp.install().then(() => {}).catch(() => {});
        sendJson(res, petApp.status());
      });
      registerRoute(ctx, "/pets/app/launch", (req, res) => {
        if (req.method !== "POST") {
          res.writeHead(405);
          res.end();
          return;
        ...
        try {
          sendJson(res, petApp.launch());
        } catch (error) {
          sendJson(res, { ...petApp.status(), error: error?.message ?? String(error) }, 400);
        }
      });
      registerRoute(ctx, "/pets/app/stop", (req, res) => {
        if (req.method !== "POST") {
          res.writeHead(405);
          res.end();
          return;
        }
        sendJson(res, petApp.stop());
      });
      ...
      // ---- Update check: registry version vs installed, one-click update ----
      // `ctx.baseUrl` is the profile directory (file: URL), where `pnpm add`
      // must run. A `file:` install (local development checkout) must not offer
      // the button, since an npm update would replace the working tree link.
      let profileDir = process.cwd();
      try {
        if (ctx.baseUrl != null) profileDir = fileURLToPath(ctx.baseUrl);
      } catch {
        /* fall back to cwd when baseUrl is missing or malformed (e.g. tests) */
      }

      registerRoute(ctx, "/settings-pro/update", async (req, res) => {
        if (req.method !== "GET" && req.method !== "HEAD") {
          res.writeHead(405);
          res.end();
          return;
        ...
        try {
          // Daily-cached check (the manual "Check" button forces a fresh poll).
          const force = new URL(req.url ?? "", "http://localhost").searchParams.get("force") === "1";
          sendJson(res, await updateSnapshot(profileDir, { force }));
        } catch (error) {
          sendJson(res, { error: error?.message ?? String(error) }, 500);
        }
      });

      registerRoute(ctx, "/settings-pro/update/apply", async (req, res) => {
        if (req.method !== "POST") {
          res.writeHead(405);
          res.end();
          return;
        ...
        try {
          const result = await applyUpdate(profileDir);
          sendJson(res, { ok: true, ...result });
          // Let the response flush, then relaunch to load the new version.
          setTimeout(() => restartProcess(), 400);
        } catch (error) {
          sendJson(res, { ok: false, error: error?.message ?? String(error) }, 400);
        }
      });

      // Startup log: surface an available update once per boot (best-effort).
      updateSnapshot(profileDir).then((snap) => {
        if (snap.hasUpdate) {
          console.log(`[${name}] update available: v${snap.latest} (installed v${snap.current}); run \`pnpm add ${"@kazecreator/dsh-settings-pro"}@latest\` or use the Settings Pro banner`);
        } else if (snap.checkFailed) {
          console.log(`[${name}] update check skipped (registry unreachable)`);
        }
      }).catch(() => {
        /* silent */
      });
      ...
      registerRoute(ctx, "/settings-pro/status", (req, res) => {
        ...
        sendJson(res, { ok: true, plugin: name, storageDir: dir, lang: readUiLang(ctx) });
      });
