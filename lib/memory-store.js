...
export class MemoryStore {
  ...
  #appendDigest(digest, notes) {
    const block = notes
      .map((n) => {
        const when = new Date(n.ts).toLocaleString("en-US", { hour12: false });
  ...
  summaryText() {
    ...
    for (const date of this.listDays().reverse()) {
      const day = this.#loadDay(date);
      if (day.digest) lines.push(`[${date} earlier records]\n${day.digest}`);
      ...
      for (const note of notes) {
        const when = new Date(note.ts).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" });
        lines.push(`- [${date} ${when}] ${note.text}`);
        if (lines.length >= MAX_INJECT_NOTES) break;
      }
      if (lines.length >= MAX_INJECT_NOTES) break;
    }
    if (lines.length > 0) parts.push("Recent notes:\n" + lines.join("\n"));
  ...
  exportMarkdown({ from = "", to = "" } = {}) {
    ...
    else if (dateFrom) title = `from ${dateFrom}`;
    else if (dateTo) title = `to ${dateTo}`;
    else title = "All";

    const header = [`# Memory · ${title}`, ""];
    const summary = this.getSummary();
    ...
    for (const date of days.reverse()) {
      const day = this.#loadDay(date);
      const block = [`## ${date}`];
      if (day.digest) block.push("", `> earlier records (merged ${day.compacted} items)`, "", `> ${day.digest.replace(/\n/g, "\n> ")}`);
    ...
    return [...header, ...(sections.length > 0 ? sections : ["(no records)"]), ""].join("\n");
  }
