...
export class PetAppManager {
  ...
  launch() {
    ...
    if (process.platform === "darwin") {
      // Self-heal: refresh the bundled app files (main.js / icon / …) so a
      // plugin update takes effect on the next launch without a manual
      // re-install, and kill any stale instance before opening a fresh one.
      try {
        const target = appDir(this.dir);
        const src = templateDir();
        for (const f of TEMPLATE_FILES) {
      ...
      try { execFileSync("pkill", ["-f", appDir(this.dir)], { stdio: "ignore" }); } catch { /* nothing running */ }
