import { existsSync, mkdirSync, readFileSync, writeFileSync, rmSync, cpSync, lstatSync } from "node:fs";
...
export class PetAppManager {
  ...
  #writeBundle(srcApp, bundle) {
    ...
    // Electron.app contains symlinked frameworks (e.g. Mantle.framework's
    // Versions/Current). Copying with verbatimSymlinks preserves those links;
    // the default dereferencing turns them into files, which breaks the
    // frameworks and makes codesign fail (and the app crash/never appear).
    cpSync(srcApp, bundle, { recursive: true, verbatimSymlinks: true });
    ...
    // frameworks ("unsealed contents"), while --preserve-metadata re-signs
    // cleanly once the framework symlinks have been preserved above.
    this.#signBundle(bundle);
  }
  ...
  #signBundle(bundle) {
    try {
      ...
      throw new Error(`failed to re-sign ${bundle}: ${error?.message ?? error}`);
    }
  }

  #macBundleNeedsRebuild() {
    const bundle = this.#bundlePath();
    if (!this.installed()) return true;
    // 0.5.27 copied Electron.app with cpSync's default dereferencing, which
    // turns framework symlinks into real files and makes the renderer crash.
    // Rebuild those bundles so the symlinks (and valid signature) are restored.
    try {
      execFileSync("codesign", ["-d", bundle], { stdio: "ignore" });
    } catch {
      return true;
    }
    const mantle = join(bundle, "Contents", "Frameworks", "Mantle.framework", "Mantle");
    try {
      if (!lstatSync(mantle).isSymbolicLink()) return true;
    } catch {
      return true;
    }
    return false;
  ...
  launch() {
    const s = this.status();
    if (s.status === "running" && this.#running() && (process.platform !== "darwin" || !this.#macBundleNeedsRebuild())) return s;
    if (process.platform === "darwin") {
      ...
      // Also rebuild bundles from 0.5.27 whose framework symlinks were
      // dereferenced during the copy (those crash/never appear).
      if (this.#macBundleNeedsRebuild()) {
        const target = appDir(this.dir);
        if (!existsSync(electronBinary(target))) {
      ...
      try {
        ...
        if (changed) {
          this.#signBundle(this.#bundlePath());
        }
      } catch (error) {
        this.#setState({ status: "error", pid: null, error: `failed to update pet app bundle: ${error?.message ?? error}` });
        throw error;
      }
      this.#killMacApp();
