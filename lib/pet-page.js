...
<style>
  ...
  #bubble { position: absolute; bottom: 88px; left: 50%; transform: translateX(-50%); width: max-content; background: rgba(255,255,255,.96); border: 1px solid #e2e2e2; border-radius: 10px; padding: 5px 10px; font-size: 12px; line-height: 16px; color: #333; box-shadow: 0 2px 8px rgba(0,0,0,.15); max-width: 300px; text-align: left; }
  #body { position: absolute; left: 0; bottom: 0; width: 84px; height: 84px; display: flex; align-items: center; justify-content: center; animation: bob 2.8s ease-in-out infinite; }
...
(function () {
  ...
  var LABELS = { idle: '待机', thinking: '思考中', replying: '回复中', working: '工作中', goal: '有目标', failed: '失败', success: '完成', paused: '已暂停' };
  ...
  function statusHtml(d) {
    ...
    } else if (d.currentTodo) {
      parts.push('<div class="goal">📋 ' + esc(String(d.currentTodo).trim()) + '</div>');
    }
  ...
  function render(expr, html) {
    ...
    // Only rebuild the sprite/body when the expression or the active pet
    // actually changed. Rebuilding on every 5s poll made the pet flash.
    if (expr !== lastExpr || activeId !== lastActiveId) {
      renderBody(expr);
      lastExpr = expr;
