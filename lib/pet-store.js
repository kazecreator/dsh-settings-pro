...
import { installPet as installCodexPet, listInstalled as listCodexPets, removePet as removeCodexPet } from "./codex-pets.js";
...
export class PetStore {
  ...
  list() {
    return [...this.listCodex(), ...this.listUser()];
  }
  ...
  active() {
    ...
    return this.get(clean) != null ? clean : "";
  }

  setActive(id) {
    const clean = sanitizeId(id);
    if (clean !== "" && this.get(clean) == null) throw new Error(`Pet not found: ${id}`);
  ...
  installCodex(id, onProgress) {
    return installCodexPet(this.dir, id, onProgress);
  }

  removeCodex(id) {
    return removeCodexPet(this.dir, id);
  }
  ...
  addFromSteps({ name, states }) {
    ...
    if (!files.idle) throw new Error("Missing required idle state");
  ...
  addFromZip(buffer) {
    ...
    if (manifestBuf == null) throw new Error("Zip is missing manifest.json");
    let m;
    try {
    ...
    for (const key of STATES) {
      ...
      if (data == null) throw new Error(`Zip is missing state file ${fileName}`);
      files[key] = fileName;
  ...
  removeUser(id) {
    ...
    if (this.active() === clean) this.setActive("");
    return clean;
  }

  /** Remove any pet (codex or user), resetting the active pet if needed. */
  remove(id) {
    const clean = sanitizeId(id);
    const pet = this.get(clean);
    if (pet == null) throw new Error("Pet not found");
    if (pet.source === "codex") this.removeCodex(clean);
    else this.removeUser(clean);
    return clean;
  }
  ...
  #client(pet) {
    ...
    return { id: pet.id, name: pet.name, nameZh: pet.nameZh ?? null, source: pet.source, states };
  }
  ...
  #writeUserFile(id, fileName, buf) {
    if (buf.length > MAX_IMAGE_BYTES) throw new Error(`${fileName} exceeds the 4MB limit`);
