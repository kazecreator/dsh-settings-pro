...
function saveOffset(offset) {
  try {
    ...
    console.error("[settings-pro:im] failed to save telegram offset:", error?.message ?? error);
  }
}
...
export class TelegramChannel {
  ...
  async #call(method, body, signal) {
    ...
    for (;;) {
      ...
      if (res.status === 429 && attempt < TELEGRAM_MAX_RETRIES) {
        ...
        attempt += 1;
        console.warn(`[settings-pro:im] telegram ${method} rate-limited (429); retrying in ${delayMs}ms (attempt ${attempt}/${TELEGRAM_MAX_RETRIES})`);
  ...
  start() {
    ...
    console.log("[settings-pro:im] telegram channel starting (long polling)");
    ...
    this.#poll().catch((error) => {
      ...
      console.error("[settings-pro:im] telegram poll stopped:", error);
      this.#status?.setTelegram({ connected: false, error: error?.message ?? String(error) });
    });
    this.#sendRestartNotice().catch((error) => {
      console.error("[settings-pro:im] failed to send telegram restart notice:", error?.message ?? error);
    });
  }
  ...
  async #poll() {
    while (!this.#abort.signal.aborted) {
      try {
        ...
        if (conflict) {
          console.warn(`[settings-pro:im] telegram getUpdates conflict (another poller is active); backing off ${delay}ms before retry ${this.#pollFailures}`);
        } else {
          console.error(`[settings-pro:im] telegram poll error (retrying in ${delay}ms):`, error?.message ?? error);
        }
        this.#status?.setTelegram({ connected: false, error: error?.message ?? String(error) });
  ...
  #handleMessage(message) {
    ...
    if (allowed.length > 0 && !allowed.includes(fromId) && !allowed.includes(chatId)) {
      console.log(`[settings-pro:im] telegram: dropped message from ${fromId} (not in allowlist)`);
      return;
    }
    ...
    console.log(`[settings-pro:im] telegram: message from ${fromId}: ${photo != null ? "[photo]" : ""} ${text.slice(0, 120)}`);
    ...
    run().then((replyText) => {
      console.log(`[settings-pro:im] telegram: replied to ${chatId}: ${replyText.slice(0, 120)}`);
    }).catch((error) => {
      console.error("[settings-pro:im] telegram reply failed:", error);
...
class TelegramReplyStreamer {
  ...
  async #send(md) {
    ...
    if (html !== "") {
      ...
      try {
        ...
        console.warn("[settings-pro:im] telegram HTML send failed, falling back to plain text:", error?.message ?? error);
      }
    }
    ...
    try {
      ...
      console.warn("[settings-pro:im] telegram plain sendMessage failed:", error?.message ?? error);
      return null;
    }
  }
    ...
    try {
      ...
      if (html !== "") {
        try {
          ...
          console.warn("[settings-pro:im] telegram editMessageText (plain fallback) failed:", fallbackError?.message ?? fallbackError);
        }
      } else {
        console.warn("[settings-pro:im] telegram editMessageText failed:", error?.message ?? error);
      }
    }
  }
  ...
  async #editPlain(messageId, text) {
    ...
    try {
      ...
      if (/not modified/i.test(error?.message ?? "")) return;
      console.warn("[settings-pro:im] telegram plain editMessageText failed:", error?.message ?? error);
    }
  }
    ...
    this.#activityChain = this.#activityChain.then(async () => {
      if (this.#statusId == null) {
        ...
        await this.#call("editMessageText", { chat_id: this.#chatId, message_id: this.#statusId, text: `⏳ ${label}…` }).catch((error) => {
          console.warn("[settings-pro:im] telegram failed to update activity status:", error?.message ?? error);
        });
      }
    }).catch((error) => {
      console.warn("[settings-pro:im] telegram activity status send failed:", error?.message ?? error);
    });
  ...
  async onFinal(text) {
    ...
    if (this.#statusId != null) {
      await this.#call("deleteMessage", { chat_id: this.#chatId, message_id: this.#statusId }).catch((error) => {
        console.warn("[settings-pro:im] telegram failed to delete status message:", error?.message ?? error);
      });
      this.#statusId = null;
    }
  }
