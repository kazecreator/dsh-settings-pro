...
export class UsageService {
  ...
  async backfillOfficial(token, months = 3) {
    ...
    if (raw === "") throw new Error("请填入 platform 的 userToken");
    const headers = this.#platformHeaders(raw);
    const getJson = async (url) => {
      const res = await fetch(url, { headers });
      if (res.status === 401) throw new Error("平台 token 无效或已过期");
      ...
      if (codes.includes(40002) || codes.includes(40003)) {
        throw new Error("平台 token 已过期，请重新登录 platform.deepseek.com 后重新同步");
      }
      if (typeof body?.code === "number" && body.code !== 0) {
        throw new Error(`平台接口错误 code=${body.code}`);
      }
      return body;
    };
