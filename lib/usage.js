...
export class UsageService {
  ...
  constructor(ctx, config, dir) {
    ...
    this.sync = { state: "idle", reason: "", detail: "", lastSyncedAt: 0 };
  ...
  /**
   * Read a platform response body as JSON. `res.json()` throws the cryptic
   * "Unexpected end of JSON input" when the body is empty, and a WAF challenge
   * page yields "Unexpected token '<'". This reads the raw text so empty bodies
   * become `null` (caller decides) and non-JSON bodies become clear errors.
   */
  async #readPlatformJson(res) {
    const trimmed = String(await res.text() ?? "").trim();
    if (trimmed === "") return null;
    if (trimmed[0] !== "{" && trimmed[0] !== "[") {
      const blocked = /<html|<!doctype/i.test(trimmed);
      throw new Error(
        blocked
          ? `平台接口返回了拦截页（HTTP ${res.status}），请稍后重试，或重新登录 platform.deepseek.com`
          : `平台接口返回了非 JSON 内容（HTTP ${res.status}）`,
      );
    }
    try {
      return JSON.parse(trimmed);
    } catch {
      throw new Error(`平台接口返回了无法解析的 JSON（HTTP ${res.status}）`);
    }
  }
  ...
  async #platformTokenValid(token) {
    ...
    try {
      ...
      const body = await this.#readPlatformJson(res);
      // Empty body can't confirm the token; treat it as invalid rather than
      // letting a later backfill run on a bogus token.
  ...
  async backfillOfficial(token, months = 3) {
    ...
    const getJson = async (url) => {
      ...
      const body = await this.#readPlatformJson(res);
      if (body == null) {
        throw new Error("平台用量接口返回了空响应（HTTP 200）；请重新登录 platform.deepseek.com 后重试");
      }
  ...
  /**
   * Guarded auto-sync: never overlap, and record the outcome so the settings
   * panel can explain why official usage is missing and point to re-sign-in.
   */
  async syncOfficial(months = AUTO_BACKFILL_MONTHS) {
    if (!this.#enabled) return;
    if (this.backfilling) return;
    this.backfilling = true;
    this.sync = { ...this.sync, state: "syncing", reason: "", detail: "" };
    try {
      const ok = await this.autoBackfill(months);
      if (!ok) {
        this.markSyncError("no-session");
        return;
      }
      this.markSyncOk();
    } catch (error) {
      // Best-effort: clear the cached token so the next tick re-detects (the
      // session may have expired), and keep the reason for the settings panel.
      this.platformToken = "";
      const message = error?.message ?? String(error);
      this.markSyncError(/invalid|expired|401|40002|40003/i.test(message) ? "token-expired" : "failed", message);
    } finally {
      this.backfilling = false;
    }
  }
  ...
  start() {
    // Official billed usage is the single source of truth — no local token
    // estimation anymore. Just schedule the balance poll and official backfill.
  ...
  /** Mark the sync as succeeded (called by both the timers and manual sync). */
  markSyncOk() {
    this.sync = { state: "ok", reason: "", detail: "", lastSyncedAt: Date.now() };
  }

  /** Mark the sync as failed with a machine-readable reason. */
  markSyncError(reason, detail = "") {
    this.sync = { state: "error", reason, detail };
  }

  /** Public snapshot of the sync outcome, for the settings panel. */
  syncSnapshot() {
    return {
      state: this.sync.state,
      reason: this.sync.reason,
      detail: this.sync.detail,
