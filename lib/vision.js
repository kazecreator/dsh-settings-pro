...
// Vision endpoint / model / key env start EMPTY: the panel's provider dropdown
// fills them in, and `describe()` fails with a clear message until they are
// configured. There is no MiniMax default — the user explicitly picks a VLM.
 ...
 * they are added back with that endpoint explicitly. There is no implicit
 * default — the panel leaves endpoint/model/key empty until a provider is chosen.
 *
...
export class VisionService {
  ...
  get baseUrl() {
    return normalizeBaseUrl(this.config.visionBaseUrl) ?? "";
  }

  get model() {
    return String(this.config.visionModel ?? "").trim();
  }

  get apiKeyEnv() {
    return String(this.config.visionApiKeyEnv ?? "").trim();
  }
  ...
  async describe(image, { mediaType = "image/png", question = "", lang = "en" } = {}) {
    if (this.baseUrl === "") {
      throw new Error("视觉未配置端点：请在「设置 Pro → 视觉」选择 provider 并保存");
    }
    if (this.model === "") {
      throw new Error("视觉未配置模型：请在「设置 Pro → 视觉」选择模型并保存");
    }
  ...
  async listVisionProviders() {
    ...
    // Well-known OpenAI-compatible providers (MiniMax, OpenAI, Anthropic, …) are
    // offered before the pi-ai catalog because their OpenAI routes are not in
    // that catalog; ordering here is a dropdown convenience, not a default.
