 ...
 * Credentials persist to `$DSH_HOME/storages/dsh-settings-pro/im/wechat.json`
...
function saveCredentials(creds) {
  try {
    ...
    console.error("[settings-pro:im] failed to save wechat credentials:", error);
  }
}
...
function saveCursor(buf) {
  try {
    ...
    console.error("[settings-pro:im] failed to save wechat cursor:", error?.message ?? error);
  }
}
...
function extractImage(message) {
  ...
  for (const item of items) {
    ...
    console.log(`[settings-pro:im] weixin: unhandled non-text item shape: ${JSON.stringify(item).slice(0, 500)}`);
  }
  return null;
}
...
export class WeChatChannel {
  ...
  async start() {
    ...
    if (this.#creds && this.#creds.botToken) {
      ...
      this.#sendRestartNotice().catch((error) => {
        console.error("[settings-pro:im] failed to send wechat restart notice:", error?.message ?? error);
      });
    } else {
      this.#startLogin().catch((error) => {
        console.error("[settings-pro:im] weixin login failed:", error);
  ...
  async #startLogin() {
    console.log("[settings-pro:im] weixin: fetching bot QR code...");
    const res = await postJson(BASE_URL, `ilink/bot/get_bot_qrcode?bot_type=${encodeURIComponent(BOT_TYPE)}`, {
      local_token_list: [],
    }, null, 15000);
    console.log("[settings-pro:im] weixin: QR response received, rendering...");
    ...
    console.log("[settings-pro:im] weixin: QR ready, waiting for scan");
  ...
  async #pollLogin(qrcode) {
    while (!this.#abort.signal.aborted) {
      ...
      console.log(`[settings-pro:im] weixin: QR status = ${st.status ?? "wait"}`);
  ...
  #startPolling() {
    ...
    this.#poll().catch((error) => {
      console.error("[settings-pro:im] weixin poll stopped:", error);
    ...
    while (!this.#abort.signal.aborted) {
      try {
        ...
        if (error?.name !== "AbortError") {
          console.error("[settings-pro:im] weixin getUpdates error (retrying):", error?.message ?? error);
        }
  ...
  #handleMessage(message) {
    ...
    const peerId = contextToken != null && String(contextToken) !== ""
      ? `${fromId}:${contextToken}`
      : fromId;
    console.log(`[settings-pro:im] weixin: message from ${fromId} (ctx ${contextToken ?? "-"}): ${imageRef != null ? "[image]" : ""} ${text.slice(0, 120)}`);
    ...
    run().catch((error) => {
      console.error("[settings-pro:im] weixin reply failed:", error);
      typing.stop();
  ...
  async #getTypingTicket(userId, contextToken) {
    ...
    try {
      ...
      console.warn("[settings-pro:im] weixin getconfig failed:", error?.message ?? error);
  ...
  #beginTyping(userId, contextToken) {
    ...
    (async () => {
      try {
        ...
        console.warn("[settings-pro:im] weixin typing failed:", error?.message ?? error);
      }
    })();
